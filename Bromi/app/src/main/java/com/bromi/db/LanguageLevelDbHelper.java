...
public class LanguageLevelDbHelper extends SQLiteOpenHelper {
     ...
     * - Only works for "german" right now
     *
    ...
    public static final String SQL_CREATE_ENTRIES =
            "CREATE TABLE IF NOT EXISTS " + Entries.TABLE_GERMAN
                    + " ("
                    + Entries._ID + " INTEGER PRIMARY KEY" + COMMA_SEP
                    + Entries.COLUMN_LEVEL_ID + INTEGER_TYPE + COMMA_SEP
                    + Entries.COLUMN_QUESTION_TYPE_ID + TEXT_TYPE + COMMA_SEP
                    + Entries.COLUMN_FOREIGN_WORD + TEXT_TYPE + COMMA_SEP
                    + Entries.COLUMN_TRANSLATED_WORD + TEXT_TYPE
                    + ")";
    ...
    public void insertLevelData() {
        ...
        for (int i = 0; i < LanguageLevelData.level_count; i++) {
            for (int j = 0; j < LanguageLevelData.level_length; j++) {
                db.execSQL(
                        "INSERT INTO " + Entries.TABLE_GERMAN + " ("
                        + Entries.COLUMN_LEVEL_ID + ", "
                        + Entries.COLUMN_FOREIGN_WORD + ", "
                        + Entries.COLUMN_TRANSLATED_WORD + ") "
                        + "VALUES (" + i + ", '" + LanguageLevelData.levels_german[i][j] + "', '" + LanguageLevelData.levels_english[i][j] + "');");
            }
        }
    }

    /**
     * Get a level from the database by levelId
    ...
    public Cursor getLevel(int levelId) {
        ...
        return db.rawQuery(
                "SELECT " + Entries.COLUMN_FOREIGN_WORD + "," + Entries.COLUMN_TRANSLATED_WORD
                        + " FROM " + Entries.TABLE_GERMAN
                        + " WHERE " + Entries.COLUMN_LEVEL_ID + " = ?", new String[] { String.valueOf(levelId) } );
    }

    /**
     * Return a column row from a Cursor object (of e.g. getLevel()) as a HashMap
     * @param c
     * @return
     */
